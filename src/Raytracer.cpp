...
void RayTracer::Raytracer::runWithoutUpscaling(const int samplesPerPixel, const int imageWidth, const int imageHeight)
{
    ...
    std::cerr << "Done writing to file with " << samplesPerPixel << " samples per pixel." << std::endl;
}

void RayTracer::Raytracer::runWithUpscaling(const int imageWidth, const int imageHeight)
{
    for (int j = 1; j <= _camera.getSamplesPerPixel(); j *= 2)
        runWithoutUpscaling(j, imageWidth, imageHeight);
}
...
void RayTracer::Raytracer::run()
{
    ...
    const bool upscale = _camera.getUpscale();

    if (upscale)
        runWithUpscaling(imageWidth, imageHeight);
    else
        runWithoutUpscaling(_camera.getSamplesPerPixel(), imageWidth, imageHeight);
